...
// The generic contents write behind putRecord and putRecordUpdate, for the
// registry's non-record files (the deploy-token registry writes
// tokens/<login>.json through this). Same CAS discipline as putRecordUpdate:
// a sha must echo the read, and GitHub refuses a write over a file that moved
// underneath it. Only the absence of a sha makes an unchecked create, which
// is exactly what the claim path wants and nobody else should rely on.
export async function putContents(path, data, { sha, fetchImpl = fetch, token, message } = {}) {
  const body = {
    message,
