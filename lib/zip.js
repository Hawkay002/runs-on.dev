...
export function readZip(buffer, {
  ...
  for (let i = 0; i < entries; i++) {
    if (pos + 46 > eocd || buffer.readUInt32LE(pos) !== CD_SIG) return fail('corrupt');
    ...
    if (pos + 46 + nameLen > eocd) return fail('corrupt');
  ...
  for (const file of files) {
    ...
    // A zero CRC in the central directory marks a writer that streamed the
    // zip without backfilling it; there is nothing to check in that case.
    if (file.crc !== 0 && crc32(data) !== file.crc) return fail('corrupt');
    budget -= data.length;
